...
class EffectiveElasticTensor:
    ...
    def write_constants_to_file(self, n_start, n_end, n_step):
        coefficients = self.compute_effect_elast_constants_voigt()
        file_path = (self.output_dir_of_file_with_tensor + "/" +
                     str(self.sample) + "_young(" + str(self.Y1) + "," + str(self.Y2)
                     + ")_n(" + str(n_start) + "," + str(n_end) + "," + str(n_step) + ").txt")
